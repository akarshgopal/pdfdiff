...
function pageFor(pdf: LoadedPdf, pageNumber: number): Promise<PDFPageProxy> {
  if (!Number.isInteger(pageNumber) || pageNumber < 1 || pageNumber > pdf.pageCount) {
    return Promise.reject(
      new RangeError(`Page ${pageNumber} is outside the document's 1-${pdf.pageCount} range.`),
    );
  }
...
async function renderIntoCanvas(
  ...
  const renderTask = page.render({
    ...
    background,
    annotationMode:
      options.includeAnnotations === false ? AnnotationMode.DISABLE : AnnotationMode.ENABLE,
  });
...
/** Render one page into a bounded, normalized canvas. */
...
/**
 * Render corresponding pages to the same pixel grid. The larger page defines
 * the normalized canvas; the smaller page is centered and padded with the
 * requested background color. This keeps scale and coordinates stable for
 * pixel diffing while still allowing different PDF page sizes.
 */
export async function renderPagePair(
  ...
  // The shared canvas is bounded by maxPixels (with a possible final rounding
  // pixel) and each side is rendered at exactly the same scale.
