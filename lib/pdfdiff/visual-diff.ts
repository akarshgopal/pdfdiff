...
import { findChangeRegions } from "./regions";
import type {
  ChangeRegion,
  RenderedPage,
  RgbColor,
...
/**
 * Compare two equal-size RGBA images. Pixelmatch supplies threshold and
 * antialias filtering; the second pass creates a stable directional red/cyan
 * overlay and a compact mask for region navigation.
 */
export function diffImages(
  ...
  if (earlier.data.length !== total * 4 || newer.data.length !== total * 4) {
    throw new RangeError("ImageData buffers do not match their dimensions.");
  }
  throwIfAborted(options.signal);

  const changedMask = new Uint8Array(total);
  // pixelmatch's diffMask leaves unchanged pixels transparent and writes a
  // non-transparent value for accepted differences.
  ...
  for (let index = 0; index < total; index += 1) {
    ...
    // New darker ink is conventionally an addition; old darker ink is a
    // removal. Equal-luminance color changes use the neutral "both" code.
  ...
  const overlay = new ImageData(overlayData, width, height);
  const regions: readonly ChangeRegion[] = findChangeRegions(changedMask, width, height, {
...
export function diffRenderedPages(
  earlier: RenderedPage,
  newer: RenderedPage,
  options: VisualDiffOptions = {},
): VisualDiffResult {
  return diffImages(earlier.imageData, newer.imageData, options);
}
