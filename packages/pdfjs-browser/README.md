...
Browser adapter for PDF Diff. It owns PDF.js loading, rendering, and the
default comparison engine while delegating comparison algorithms to
`@pdfdiff/core`. Text extraction lives in `@pdfdiff/pdfjs-text` and is
re-exported here.
...
Lower-level `loadPdf`, `renderPage`, `renderPagePair`, and `extractPageText`
exports are available when an application needs a custom pipeline.
